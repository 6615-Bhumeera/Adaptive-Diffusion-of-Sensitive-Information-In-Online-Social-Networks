...
from Remote_User.models import review_Model,ClientRegister_Model,adaptive_diffusion_model,recommend_Model,tweet_accuracy_model
...
def Add_DataSet_Details(request):
    if "GET" == request.method:
        ...
        for row in worksheet.iter_rows():
            ...
            adaptive_diffusion_model.objects.all().delete()
            tweet_accuracy_model.objects.all().delete()
    for r in range(1, active_sheet.max_row+1):
        adaptive_diffusion_model.objects.create(
        ...
        retweet=active_sheet.cell(r, 13).value,
        retweeter=active_sheet.cell(r, 14).value,
        retweet_date=active_sheet.cell(r, 15).value
        )
...
def Search_Tweet_Details(request):
    ...
    if request.method == "POST":
        ...
        return render(request, 'RUser/Search_Tweet_Details.html',{'objs': obj})
    return render(request, 'RUser/Search_Tweet_Details.html')

def ratings(request,pk):
    vott1, vott, neg = 0, 0, 0
    objs = adaptive_diffusion_model.objects.get(id=pk)
    unid = objs.id
    vot_count = adaptive_diffusion_model.objects.all().filter(id=unid)
    for t in vot_count:
        vott = t.ratings
        vott1 = vott + 1
        obj = get_object_or_404(adaptive_diffusion_model, id=unid)
        obj.ratings = vott1
